...
$(document).ready(() => {
	...
	$('#dash-today h3').text(
		now == today.format('E') ? 'Today' : today.format('dddd')
	)
	$('#dash-tomorrow h3').text(
		now + 1 == tomorrow.format('E') ? 'Tomorrow' : tomorrow.format('dddd')
	)
	...
	$.ajax({
		...
		url:
			document.baseURI +
			'/events/?start=' +
			today.startOf('day').format('X') +
			'&end=' +
			tomorrow.endOf('day').format('X'),
		dataType: 'json'
	}).done(data => {
